...
第2段階で戦闘画面描画、戦闘計算、AI学習、AI推論、戦闘データ読込を独立責務へ分けた。第3段階では固定モブと非表示エンカウント敵の状態機械を `KadokaQuest` から分離した。ゲームセッションは移動、イベント、保存、画面遷移などの互換窓口をまだ持つため、残る責務も段階的に分ける。開発予定の「ロジックの整理」は、その分割が終わるまで完了扱いにしない。
