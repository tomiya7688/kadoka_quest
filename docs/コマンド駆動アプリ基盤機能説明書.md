...
第2段階で戦闘の5責務、第3段階で固定モブと非表示敵、第4段階で主人公移動・イベント解釈・進行保存・マップ読込を分離した。第5段階では `RuntimeOrchestrator` が画面モードと4つのコマンド対象を所有し、第6～9段階で戦闘進行、暗号入力、画像取得、管理プロセスを専用クラスへ移した。第10～13段階でpygame入力とフィールド簡易パーティ操作を分離し、第14段階では `MonsterImportService` が個体再走査と模擬戦用外部個体読込を所有する。残るゲーム進行処理も段階的に分ける。開発予定の「ロジックの整理」は、その分割が終わるまで完了扱いにしない。
