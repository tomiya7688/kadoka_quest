...
第2段階で戦闘の5責務、第3段階で固定モブと非表示敵、第4段階で主人公移動・イベント解釈・進行保存・マップ読込を分離した。第5段階では `RuntimeOrchestrator` が画面モードと4つのコマンド対象を所有し、フィールド効果から別アプリへの接続もコマンド化した。第6段階では `BattleSession` が戦闘進行、第7段階では `PasswordSession` が暗号入力、第8段階では `CharacterImageProvider` が画像取得とキャッシュを所有する。管理ツールのプロセス状態なども段階的に分ける。開発予定の「ロジックの整理」は、その分割が終わるまで完了扱いにしない。
