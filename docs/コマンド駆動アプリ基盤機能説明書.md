...
第2段階で戦闘の5責務、第3段階で固定モブと非表示敵、第4段階で主人公移動・イベント解釈・進行保存・マップ読込を分離した。第5段階では `RuntimeOrchestrator` が画面モードと4つのコマンド対象を所有し、第6～9段階で戦闘進行、暗号入力、画像取得、管理プロセスを専用クラスへ移した。第10段階では `RuntimeInputAdapter` がpygameキーイベントを、第11段階では `RuntimeMouseAdapter` が画面固有クリックを意味コマンド要求へ変換する。フィールド操作選択なども段階的に分ける。開発予定の「ロジックの整理」は、その分割が終わるまで完了扱いにしない。
