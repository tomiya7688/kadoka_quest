...
第2段階で戦闘の5責務、第3段階で固定モブと非表示敵の状態機械を分離した。第4段階では主人公移動、フィールドイベント解釈、進行保存、マップ読込も `KadokaQuest` から分けた。ゲーム本体には戦闘開始・暗号入力・管理画面などアプリ間の効果を接続する互換窓口が残るため、残る責務も段階的に分ける。開発予定の「ロジックの整理」は、その分割が終わるまで完了扱いにしない。
