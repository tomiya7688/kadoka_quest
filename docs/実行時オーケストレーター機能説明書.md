...
戦闘演出、暗号入力、画像取得、管理プロセスは第6～9段階で専用クラスへ移した。第10～11段階ではpygameのキーとクリックを意味コマンド要求へ変換し、第12段階では `FieldPartySession` がフィールド上の選択カーソルを所有する。`FieldCommandApplication` は `party.select` をセッションの公開操作へ渡し、ゲームやUIの属性を直接書き換えない。
