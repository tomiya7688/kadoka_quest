...
戦闘演出、暗号入力、画像取得、管理プロセスは第6～9段階で専用クラスへ移した。第10～11段階ではpygame入力を意味コマンド要求へ変換し、第12～13段階では `FieldPartySession` と `FieldPartyService` がフィールド簡易パーティの選択状態と永続操作を分担する。`FieldCommandApplication` は意味コマンドを公開操作へ渡し、UI属性や保存ファイルを直接変更しない。
