...
戦闘演出は第6段階で `BattleSession`、暗号入力は第7段階で `PasswordSession`、画像取得とキャッシュは第8段階で `CharacterImageProvider` へ移した。管理ツールのプロセス状態はまだ `KadokaQuest` にある。次段階では外部管理ツールの起動・終了検知を専用サービスへ移す。
