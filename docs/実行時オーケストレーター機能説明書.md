...
戦闘演出、暗号入力、画像取得、管理プロセスは第6～9段階で専用クラスへ移した。第10～13段階ではpygame入力とフィールド簡易パーティ操作を分離し、第14段階では `MonsterImportService` が外部個体フォルダを扱う。`FieldCommandApplication` は `acquire.scan / simulation.start` をゲームの互換操作へ配送し、ゲームはサービス結果に応じた画面遷移と戦闘演出開始だけを行う。
