...
戦闘演出は第6段階で `BattleSession`、暗号入力は第7段階で `PasswordSession`、画像取得は第8段階で `CharacterImageProvider`、管理ツールのプロセス状態は第9段階で `ManagerProcessService` へ移した。次段階ではpygameキーイベントから意味コマンドへの変換を入力アダプターとして分離する。
